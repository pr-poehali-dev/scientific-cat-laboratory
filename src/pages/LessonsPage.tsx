  ...
  return (
    <>
      <button
        ...
        style={{
          background: expanded
            ? `linear-gradient(135deg, ${lesson.color}25, rgba(34,21,53,0.95))`
            : 'rgba(34,21,53,0.7)',
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div className="shrink-0 text-lg opacity-50" style={{ color: 'var(--sand-light)' }}>
            {expanded ? '▲' : '▼'}
          </div>
        </div>
      </button>
      ...
      {expanded && (
        ...
        >
          {/* Top bar */}
          <div className="flex items-center gap-3 px-4 pt-safe pt-4 pb-3 shrink-0"
            style={{ borderBottom: '1px solid rgba(61,36,100,0.6)' }}>
            <button
          ...
          <div className="scrollable flex-1 px-4 py-4 space-y-4">
            ...
            <div className="glass-card rounded-2xl p-4">
              <h3 className="font-display font-semibold text-sm mb-3 tracking-wide"
                style={{ color: lesson.color }}>
                📖 ТЕОРИЯ
              </h3>
              {lesson.content.split('\n\n').map((para, i) => (
                <p key={i} className="text-sm leading-relaxed mb-3 last:mb-0"
                  style={{ color: 'rgba(245,230,200,0.88)' }}>
                  {para}
                </p>
              ))}
            </div>
            ...
            <div className="rounded-2xl p-4 relative overflow-hidden"
              style={{
                background: 'linear-gradient(135deg, rgba(191,95,255,0.12), rgba(34,21,53,0.9))',
                border: '1.5px solid rgba(191,95,255,0.4)',
              }}>
              <div className="flex gap-2 items-start">
                <span className="text-2xl shrink-0 animate-float">🐱</span>
                <div>
                  <p className="font-display text-xs tracking-widest mb-2"
                    style={{ color: 'rgba(191,95,255,0.7)' }}>
